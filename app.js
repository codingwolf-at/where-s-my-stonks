...
const calculateProfitAndLoss = () => {
  ...
  if (initialPrice < currentPrice) {
    ...
    outputDiv.innerText = `Yay!! Your Profit is ${profit.toFixed(1)} and profit Percentage is ${profitPercentage.toFixed(1)}%`;
  } else if (currentPrice < initialPrice) {
    const loss = (initialPrice - currentPrice) * stockNumber;
    const lossPercentage = (loss / (initialPrice * stockNumber)) * 100;
		outputDiv.style.color = 'red';
		outputDiv.innerText = `Whoops!! Your loss is ${loss.toFixed(1)} and loss Percentage is ${lossPercentage.toFixed(1)}% :(`;
  } else {
